...
pub fn NavBar<T: PartialEq + Clone + Routable + 'static>(props: &Props<T>) -> Html {
    ...
    html! {
        <>
        <header>
        <nav class="fixed left-0 right-0 top-0 z-[1] px-4 bg-slate-200 dark:bg-slate-800 font-semibold transition-all duration-300"
        id="nav-bar">
        <div class="flex justify-between items-center mt-3 pb-3 border-b-2 dark:border-sky-500 border-sky-600 w-auto min-[1920px]:w-[1888px] min-[1920px]:mx-auto">
            ...
            if active_menu{
                ...
                <Link<T> classes="flex gap-1 items-center text-black dark:text-white" to={props.home_route.clone()}>
                    <img class="block" id="logo-img" width="50" src={logo.clone()}
                        alt={name_web.clone()}/>
                    <p class="duration-300 hover:text-gray-700 dark:hover:text-gray-400">{name_web}</p>
                </Link<T>>
            </div>
